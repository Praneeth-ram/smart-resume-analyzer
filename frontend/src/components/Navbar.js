...
export default function Navbar() {
  ...
  const NavLink = ({ to, children }) => {
    ...
    return (
      <Link to={to} style={{
        textDecoration: 'none', fontSize: 13, fontWeight: 800, textTransform: 'uppercase', letterSpacing: 0.5,
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 40px', height: 72, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <Link to="/" style={{ display: 'flex', alignItems: 'center', gap: 12, textDecoration: 'none' }} 
          ...
          <div style={{ 
            ...
            color: '#fff', fontWeight: 900, fontSize: 20, boxShadow: '0 4px 10px rgba(124, 58, 237, 0.3)'
          }}>S</div>
          <span style={{ fontSize: 22, fontWeight: 900, color: text, letterSpacing: '-0.5px' }}>
        ...
        <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
          {!user ? (
            <>
              <Link to="/login" style={{ 
                textDecoration: 'none', fontSize: 14, fontWeight: 800, color: text,
              ...
              <Link to="/register" style={{ 
                textDecoration: 'none', fontSize: 14, fontWeight: 800, color: '#fff',
            ...
            <>
              <Link to="/hr/jobs/new" style={{ 
                textDecoration: 'none', fontSize: 13, fontWeight: 800, color: '#fff', textTransform: 'uppercase', letterSpacing: 0.5,
              ...
              <button onClick={handleLogout} style={{ 
                ...
                fontSize: 14, fontWeight: 800, color: text, padding: '10px 20px', borderRadius: '12px', transition: 'background 0.2s'
              }} onMouseEnter={e => e.target.style.background = isDarkHome ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.05)'} 
                 onMouseLeave={e => e.target.style.background = 'transparent'}>
                Sign Out
              </button>
