...
class Application(Base):
    ...
    resume_file = Column(LargeBinary, nullable=True)
    resume_mimetype = Column(String, nullable=True)
    resume_text = Column(Text, nullable=True)        # extracted plain text, used by RAG

    # ── Phase 2: RAG + LLM ────────────────────────────────────────
    rag_verdict = Column(String, nullable=True)      # "SELECTED" or "REJECTED"
    rag_score = Column(Float, nullable=True)         # semantic similarity 0.0 – 1.0
    rag_reasoning = Column(Text, nullable=True)      # LLM explanation paragraph
    rag_strengths = Column(Text, nullable=True)      # JSON array  e.g. '["Python", "FastAPI"]'
    rag_gaps = Column(Text, nullable=True)           # JSON array  e.g. '["Kubernetes"]'
    rag_recommendation = Column(Text, nullable=True) # one-line HR recommendation
